...
import { Search, Calendar, Filter, Clock, Loader2, ChevronDown, ChevronUp } from "lucide-react";
...
export default function TimeLogPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    return rawData
      .map((summary) => {
        ...
        if (searchTerm && !summary.task.title.toLowerCase().includes(searchTerm.toLowerCase())) {
          return null;
        }
        ...
        if (dateFilter !== "all") {
          filteredLogs = summary.logs.filter((log) => {
            ...
            if (dateFilter === "custom" && customDate) {
              ...
              return logDate === new Date(custom.getTime() + custom.getTimezoneOffset() * 60000).toDateString();
            }
            return true;
          });
        }
        ...
        const recalculatedTotal = filteredLogs.reduce((acc, log) => acc + (log.durationSeconds || 0), 0);
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Time Logs History</h1>
        ...
        <div className="space-y-4">
          {filteredData.length === 0 ? (
            <div className="text-center py-10 border border-dashed border-zinc-300 dark:border-zinc-800 rounded-lg">
              <p className="text-zinc-500 dark:text-zinc-400">No time logs match your hunt.</p>
            </div>
          ) : (
            filteredData.map((summary: any) => (
              <div key={summary.task.id} className="border border-zinc-200 dark:border-zinc-800 rounded-lg bg-white dark:bg-zinc-950 overflow-hidden">
                <div 
                ...
                >
                  <div className="flex-1">
                    <h3 className="font-semibold text-zinc-900 dark:text-zinc-100">{summary.task.title}</h3>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {expandedTaskId === summary.task.id ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                  </div>
                </div>
                ...
                {expandedTaskId === summary.task.id && (
                  <div className="border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/30 p-4 space-y-2">
                    {summary.logs.map((log: any) => (
                      <div key={log.id} className="flex justify-between items-center text-sm p-2 rounded-md bg-white dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800">
                        <span className="text-zinc-500">
                          {new Date(log.startTime).toLocaleString()}
                        </span>
                        <span className="font-mono text-zinc-700 dark:text-zinc-300">
                          {log.endTime ? formatTime(log.durationSeconds) : "Active..."}
                        </span>
                      </div>
                    ))}
                  </div>
